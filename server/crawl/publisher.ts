...
export async function publishRun(
  ...
  const policy = run.project.currentPolicy;
  ...
  const outcome = await withRunFence<PublishOutcome>(db, run.id, input.leaseToken, async (tx) => {
    const fence = await tx.crawlRun.findUniqueOrThrow({
      where: { id: run.id },
      select: { publishedAt: true, status: true, comparisonMode: true },
    });
